...
- **Input Surface** (`{colors.input-surface}` — `#191919`): A neutral charcoal fill dedicated to text fields, selects, and composer inputs.
...
- Background `{colors.input-surface}` (`#191919`), text `{colors.ink}`, 1 px solid `{colors.hairline}`, body in `{typography.body-md}`, padding `{spacing.md} {spacing.lg}`, shape `{rounded.sm}` 8 px.
