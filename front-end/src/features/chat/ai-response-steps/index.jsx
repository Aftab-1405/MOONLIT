...
import { TIMELINE_LINE_X, slideIn } from '@/features/chat/ai-response-steps/timelineShared';
import {
  normalizeSteps,
  buildStepsSummary,
  getCurrentStepIndex,
...
export const StepsAccordion = memo(function StepsAccordion({ steps, isStreaming }) {
  ...
  const isSingleWorkflowStep = normalizedSteps.length === 1 && normalizedSteps[0].id?.startsWith('workflow-');
  ...
  return (
    ...
    >
      ...
      <ButtonBase
        ...
        aria-label={isExpandable ? (expanded ? 'Collapse reasoning steps' : 'Expand reasoning steps') : undefined}
        sx={{
          width: '100%',
          display: 'flex',
          alignItems: 'center',
