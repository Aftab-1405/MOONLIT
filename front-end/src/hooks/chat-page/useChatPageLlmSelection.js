...
export function useChatPageLlmSelection({ settings, updateSettings }) {
  ...
  useEffect(() => {
    ...
    const providerConfig =
      providerOptions.find((provider) => provider.name === validProvider) || providerOptions[0];
    const candidateModels = providerConfig?.models || [];
    const validModel =
      settings.llmModel && candidateModels.includes(settings.llmModel)
        ? settings.llmModel
