    ...
    *,
    ...
    try:
        ...
        raise TokenCountingError(
            f"Bedrock CountTokens failed for model {model_id}: {exc}"
        ) from exc
...
def count_converse_tokens_with_fallback(
    ...
    *,
    ...
    try:
        ...
        if not _is_unsupported_count_tokens_error(exc):
            raise
...
def get_default_token_counter() -> Callable[[str], int]:
    ...
    from app.llm.providers.model_factory import get_default_model
