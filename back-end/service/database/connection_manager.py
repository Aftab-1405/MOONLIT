...
class ConnectionManager:
    ...
    def get_connection_context(self, config: dict):
        ...
        # Ensure pool exists (reuses the one created at connect_db time)
        if pool_key not in self._pools:
            with self._global_lock:
                if pool_key not in self._pools:
                    self._pools[pool_key] = self._create_pool(config, pool_key)
        ...
        try:
            ...
            try:
                ...
                adapter.return_connection_to_pool(self._pools[pool_key], conn)
                logger.debug(
                    f"Connection returned to pool {pool_key[:8]} via get_connection_context"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to return connection to pool {pool_key[:8]}: {e}"
                )
