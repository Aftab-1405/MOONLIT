...
def test_postgresql_adapter_sqli_protection():
    ...
    # 1. Test schema injection (should raise ValueError or parameterize)
    schema_methods = [
        ("get_tables_query", {}),
    ...
    for method_name, kwargs in schema_methods:
        ...
        try:
            ...
            # Correct behavior: Raises ValueError for invalid schema names
            pass

    # 2. Test table name injection (should be parameterized or raise ValueError)
    table_methods = [
...
def test_mysql_adapter_security():
    try:
    ...
    # 1. Verify schema parameterization across all methods
    schema_methods = [
    ...
    # 2. Verify table parameterization across all methods
    table_methods = [
...
def test_sqlserver_adapter_security():
    try:
...
def test_oracle_adapter_security():
    try:
...
def test_where_clause_parameterization():
    # Verify that user-supplied values used in WHERE clauses are parameterized
    # instead of formatted as raw strings.
    adapters = [
        (PostgreSQLAdapter, "%s"),
        (MySQLAdapter, "%s"),
        (SQLServerAdapter, "?"),
...
def test_order_by_allowlist_validation():
    # ORDER BY column names cannot be parameterized. They must be validated against an allowlist.
    ...
    # Valid column name should construct query correctly
    assert "ORDER BY username" in build_ordered_query("username")
    
    # Malicious injection attempt should raise ValueError
